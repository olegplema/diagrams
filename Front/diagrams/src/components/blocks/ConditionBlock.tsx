...
const ConditionBlock: React.FC<IProps> = ({ data }) => {
  return (
    <div className="bg-white border-2 border-gray-300 rounded-lg p-4 shadow-md min-w-[200px] relative">
      ...
      <Handle type="source" position={Position.Left} id="true" className="!w-3 !h-3 bg-green-500">
        <div></div>
        true
      </Handle>
      <Handle type="source" position={Position.Right} id="false" className="!w-3 !h-3 bg-red-500" >false</Handle>
    </div>
  );
};
